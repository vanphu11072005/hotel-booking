...
import { bookingService, Booking, serviceService, Service, paymentService, Payment } from '../../services/api';
...
const CheckInPage: React.FC = () => {
  ...
  const fetchPayment = async (bookingId: number) => {
    try {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data.booking.id) {
        await fetchPayment(response.data.booking.id);
      }
  ...
  const handleSelectBooking = async (selectedBooking: Booking) => {
    ...
    if (selectedBooking.id) {
      await fetchPayment(selectedBooking.id);
    }
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-gray-700">
                    ...
                    <span>{bookingItem.room?.room_type?.name || 'N/A'}</span>
                  </div>
                </div>
      ...
      {booking && (
        <>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            {loadingPayment ? (
              ...
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-2">
                <AlertCircle className="w-5 h-5 text-yellow-600 mt-0.5" />
                <div>
                  <p className="text-sm text-yellow-800 font-medium">Chưa có thông tin thanh toán</p>
                  <p className="text-sm text-yellow-700">
                    Đặt phòng này chưa có thanh toán nào được tạo.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {selectedServices.map((service) => (
                    <div key={service.service_id} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleServiceQuantityChange(service.service_id, service.quantity - 1)}
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-gray-700">Tổng tiền dịch vụ:</span>
                    <span className="text-lg font-semibold text-blue-600">{formatCurrency(calculateServiceTotal())}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Available Services */}
            <div>
              <h3 className="text-sm font-medium text-gray-700 mb-2">Chọn thêm dịch vụ:</h3>
